    ...
    def validate(self, data):
        if data.get("location"):
            ...
            if self.instance.location == location:
                raise serializers.ValidationError("Truck is already here")
        elif data.get("number"):
            number = data.get("number")

            if self.instance.number == number:
                raise serializers.ValidationError("Truck is already have this number")
...
class PatchTruckSerializer(serializers.ModelSerializer):
    location = ZipCodeField(help_text="Zip cod of truck location")
